...
<div class="card">
    ...
    <div class="card-footer text-center">
        <button id="final-print-btn" data-print-url="{{ route('anjungan.printFinal', ['subdomain' => app('tenant')->subdomain, 'pengajuanSurat' => $pengajuanSurat->id]) }}" class="btn btn-primary btn-lg">
            <i class="fas fa-print"></i> Data Sudah Benar, Lanjutkan Cetak
        </button>
        <a href="{{ route('anjungan.pilihSurat', ['subdomain' => app('tenant')->subdomain]) }}" class="btn btn-secondary btn-lg">Batalkan & Kembali</a>
    </div>
</div>
@stop
...
$(document).ready(function() {
    $('#final-print-btn').on('click', function(e) {
        ...
        const backUrl = "{{ route('anjungan.index', ['subdomain' => app('tenant')->subdomain]) }}";
        const button = $(this);
