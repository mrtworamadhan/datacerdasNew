...
<head>
    ...
    <style>
        ...
        .header h3 { margin: 0; text-decoration: underline; font-size: 14px;}
        .header p { margin: 0; }
        .sub-header { font-weight: bold; margin-top: 20px; margin-bottom: 5px; }
...
<body>
    ...
    <div class="header">
        <h3>LAPORAN PERTANGGUNGJAWABAN (LPJ)</h3>
        <p>KEGIATAN {{ strtoupper($kegiatan->nama_kegiatan) }}</p>
    </div>
    ...
    <table class="table">
        <tr>
            <td style="width: 70%;">Anggaran Diajukan (Proposal)</td>
            <td class="text-right">Rp {{ number_format($kegiatan->anggaran_biaya, 2, ',', '.') }}</td>
        </tr>
        <tr>
            <td>Total Realisasi Anggaran (Pengeluaran)</td>
            <td class="text-right">Rp {{ number_format($totalRealisasi, 2, ',', '.') }}</td>
        </tr>
        <tr style="font-weight: bold;">
            <td style="background-color: #f2f2f2;">Sisa Anggaran</td>
            <td class="text-right" style="background-color: #f2f2f2;">Rp {{ number_format($sisaAnggaran, 2, ',', '.') }}</td>
        </tr>
    </table>

    <div class="sub-header">C. RINCIAN PENGELUARAN DANA</div>
    <table class="table">
        <thead>
            <tr>
                <th>No</th>
        ...
        <tbody>
            ...
            @empty
            <tr><td colspan="4" class="text-center">Tidak ada catatan pengeluaran.</td></tr>
            @endforelse
            <tr style="font-weight: bold;">
                <th colspan="3" class="text-center">TOTAL REALISASI</th>
                <th class="text-right">Rp {{ number_format($totalRealisasi, 2, ',', '.') }}</th>
            </tr>
        </tbody>
    </table>

    <div class="sub-header">D. PENUTUP</div>
