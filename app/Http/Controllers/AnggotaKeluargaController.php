...
class AnggotaKeluargaController extends Controller
    {
        ...
        public function create(KartuKeluarga $kartuKeluarga)
        {
            ...
            $pendidikanOptions = ['Tidak/Belum Sekolah', 'SD', 'SMP', 'SMA', 'S1', 'S2', 'S3']; // Tambahkan ini
            $kewarganegaraanOptions = ['WNI', 'WNA'];
            $golonganDarahOptions = ['A', 'B', 'AB', 'O', '-'];
            $hubunganKeluargaOptions = ['Anak', 'Cucu', 'Istri', 'Menantu', 'Suami', 'Saudara', 'Kakak', 'Adik', 'Lainnya']; // Kepala Keluarga tidak ada di sini
            $statusKependudukanOptions = ['Warga Asli', 'Pendatang', 'Sementara', 'Pindah', 'Meninggal']; // Fixed: 'Meninggal' added back
            $statusKhususOptions = ['Disabilitas', 'Lansia', 'Ibu Hamil', 'Balita', 'Penerima PKH', 'Penerima BPNT', 'Lainnya']; // Contoh status khusus

            return view('admin_desa.anggota_keluarga.create', compact(
                'kartuKeluarga', 'rw', 'rt', 'jenisKelaminOptions', 'agamaOptions',
                'statusPerkawinanOptions', 'pekerjaanOptions', 'pendidikanOptions', 'kewarganegaraanOptions', // Tambahkan pendidikanOptions
         ...
         */
        public function edit(KartuKeluarga $kartuKeluarga, $anggotaId) // Menggunakan $anggotaId
        {
            $user = Auth::user();
            // Check if user has permission to access this module at all
            if (!$user->isAdminDesa() && !$user->isSuperAdmin()) {
            ...
            $kewarganegaraanOptions = ['WNI', 'WNA'];
            $golonganDarahOptions = ['A', 'B', 'AB', 'O', '-'];
            $hubunganKeluargaOptions = ['Anak', 'Cucu', 'Istri', 'Menantu', 'Suami', 'Saudara', 'Kakak', 'Adik', 'Lainnya'];
            $statusKependudukanOptions = ['Warga Asli', 'Pendatang', 'Sementara', 'Pindah', 'Meninggal']; // Fixed: 'Meninggal' added back
            ...
            return view('admin_desa.anggota_keluarga.edit', compact(
                'kartuKeluarga', 'anggotaKeluarga', 'jenisKelaminOptions', 'agamaOptions',
                'statusPerkawinanOptions', 'pekerjaanOptions', 'pendidikanOptions', 'kewarganegaraanOptions', // Tambahkan pendidikanOptions
                'golonganDarahOptions', 'hubunganKeluargaOptions', 'statusKependudukanOptions',
                'statusKhususOptions'
            ));
         ...
         */
        public function update(Request $request, KartuKeluarga $kartuKeluarga, $anggotaId) // Menggunakan $anggotaId
        {
            $user = Auth::user();
            // Check if user has permission to access this module at all
            if (!$user->isAdminDesa() && !$user->isSuperAdmin()) {
         ...
         */
        public function destroy(KartuKeluarga $kartuKeluarga, $anggotaId) // Menggunakan $anggotaId
        {
            $user = Auth::user();
            // Check if user has permission to access this module at all
            if (!$user->isAdminDesa() && !$user->isSuperAdmin()) {
